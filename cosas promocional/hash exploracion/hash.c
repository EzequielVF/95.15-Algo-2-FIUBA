...
size_t buscar_posicion(const hash_t *hash, const char *clave){
	...
	if (hash->tabla[pos].estado==0 || (hash->tabla[pos].estado == 1 && strcmp(hash->tabla[pos].clave, clave) == 0))return pos;
...
void hash_redimensionar(hash_t* hash, size_t tam_final){
	campo_t* campo_nuevo= crear_campo(tam_final);
...
bool hash_guardar(hash_t *hash, const char *clave_nueva, void *dato){
	...
	if ((hash->tam *2)/3 <= hash->cant) {
		size_t tam = (hash->tam)*2;
		hash_redimensionar(hash,tam);
	}
	return true;
}
///////////////////////////////////////////////////////////////////////////////
